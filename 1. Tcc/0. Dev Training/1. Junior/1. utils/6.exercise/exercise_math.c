    ...
    // 减法算式
    char * DLL_EXPORT minuslimited ( int limit )       // 限制范围内的减法法题, 例如: 20以内的加法, start 是外部传入的时间参数, 用来做随机数的初始化的
    { 
    ...
    // 创建指定限制条件的算式
    int DLL_EXPORT genEquation( int limit, int row, int col, char * title, char * filename ) 
    {
        FILE * fp;
        ...
        for (int i = 0; i < row; i ++ ) {
            for ( int j = 0; j < col ; j ++ ) {
                while( 1 )  {
                    ...
                    if ( rand() % 2 == 1 ) { // 单数, 用加法
                        sprintf( lbuf, "%s", addlimited( 20 ) );
                    } else {
                        sprintf( lbuf, "%s", minuslimited( 20 ) );
                    }
    ...
    void DLL_EXPORT getDaysExercise( int maxLines, int limit, char *title, int rows, int cols, char * filename, int days )
    {
        ...
        for ( int i = 0; i < days; i ++ ) {
            lines = genEquation( limit, rows, cols, title, filename ); 
            l = lines;
            printf("%d..", i);
