...
bool  HexIP2Str(unsigned int uiIPAddress, char * sIPAddress)
{
    ...
    // 4位hex字符串转整型数值 Four Hex To Int <547F> <9E4F>  "9E4F"->
    unsigned int DLL_EXPORT Hex2Int2( char * hex)
    {
        if ( !hex )
            return -1;
    ...
    // 2位hex字符串转整型数值 Hex To Int
    int DLL_EXPORT Hex2Int(char *hex)
    {
        if ( !hex )
            return -1;
    ...
    int DLL_EXPORT Char2Int( char hex ) 
    {
        if ( hex >= '0' && hex <= '9' )
            return hex - '0';
        if ( hex >= 'A' && hex <= 'F' )
    ...
    // desbuf 用来存放结果的缓冲区
    // len   desbuf 缓冲区长度
    // wstr  unicode 编码 类似 wchar_t wstr[] = {40527,0x50cf, 0x30, 0x31, 0};   每一个都是双字节
    int DLL_EXPORT unicode2str( char * desbuf, int len, wchar_t * wstr ) 
    {
        int ret = 0;
